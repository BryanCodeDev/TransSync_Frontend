...
<<<<<<< HEAD
  ...
  const verificarConexion = useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      if (resultado) {
        if (resultado.mode === 'offline') {
          setConnectionStatus('offline');
          console.log('🤖 Chatbot funcionando en modo offline');
          return true;
          ...
          // Estado de error pero no excepción
          console.warn('⚠️ Servicio de chatbot no disponible:', resultado.message || 'Error desconocido');
        ...
        // Resultado nulo o undefined (no debería pasar)
        console.error('❌ Resultado inesperado de verificarEstado:', resultado);
        setConnectionStatus('disconnected');
        return false;
      }
    } catch (error) {
      // Este catch NUNCA debería ejecutarse si verificarEstado está bien implementado
      console.error(`❌ Error inesperado verificando conexión:`, error);
      console.error('📍 Error en línea:', error.stack?.split('\n')[1]);
...
=======
>>>>>>> 02df8297d3e27fe1c15da64d99ea8c76cef49c4f
  useEffect(() => {
